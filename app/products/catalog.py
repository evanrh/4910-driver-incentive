...
class CatalogController():
    ...
    def update_price(self, listing_id, sponsor_id):
        ...
        sql = "UPDATE product SET price = %s WHERE listing_id=%s AND sponsor_id=%s RETURNING *"
        ...
        try:
            self.conn.exec(sql, vals)
            sql = "SELECT * FROM product WHERE listing_id=%s AND sponsor_id=%s"
            out = self.conn.exec(sql, (listing_id, sponsor_id))
